...
<html>
	...
	$headers[]= 'From: tiendaDAW.gg.com';
	$texto="<h3>Pedido nº $pedido </h3>";
	$productos=cargar_productos(array_keys($carrito));

	$texto.="<table border='1'><tr><th>Producto</th><th>Descripción</th><th>Unidades</th><th>";
	while  ($fila=mysqli_fetch_assoc($productos)){
		extract ($fila);
		$texto.="<tr><td>$Nombre</td><td>$Descripcion</td><td>$carrito[$CodProd]</td></tr>";
	...
	if($resul === FALSE){
		...
		$subject=crear_correo($_SESSION['carrito'],$resul,$_SESSION['usuario']["correo"]);
